...
## 6. 외국인 카드소비데이터
-
[2024.01 ~ 2025.02] 외국인 전용 선불 카드(Travely Pass) 마케팅 전략 수립 및 데이터 검증
1. 핵심 요약


관광 소비 중심의 데이터 정제 및 df_tour 데이터셋 구축: 서울 내 외국인 근로자 밀집 지역의 생활형 소비 업종(인터넷 쇼핑, 차량 관리 등)을 식별하기 위해 외국인 근로자 수와 업종별 비중 간의 선형회귀분석을 수행하였으며, 이를 통해 분석의 신뢰도를 확보했습니다.


대만 타겟 시장의 정량적 타당성 검증 및 핵심 상권 도출: 국적별 객단가 및 소비 비중 분석을 통해 대만이 전체 평균 대비 약 30% 높은 건당 결제액(약 21.2만 원)을 보유한 핵심 타겟임을 입증하고, 중구·강남·마포 등 우선 공략 상권을 규명했습니다.


K-means 군집 분석 기반의 국가 확장 및 서비스 고도화 전략 수립: 소비 패턴에 따른 국가별 군집화를 수행하여 대만과 일본을 '관광·쇼핑형' 국가군으로 분류하였으며, 이를 바탕으로 서비스 확장 전략 및 맞춤형 혜택 설계의 정량적 근거를 마련했습니다.

2. 기술 스택 (Tech Stack)


언어/환경: Python (Pandas, NumPy), Google Colab 


데이터 분석/모델링: 선형회귀분석(Linear Regression), K-means Clustering 


시각화/BI: Seaborn (Heatmap), Matplotlib (Bubble Chart), SQL,
